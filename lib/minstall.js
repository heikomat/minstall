...
let linkOnly = false;
let assumeLocalVersionsSatisfyNonSemverDependencyVersions = false;
    ...
    for (const dependency in module.dependencies) {
      ...
      if (!versionFound) {
        if (requestedDependencies[dependency][module.dependencies[dependency]] !== undefined) {
          ...
          // This is the first time we see that dependency-version. Let's make sure we get its actual version
...
function removeAlreadySatisfiedDependencies(requestedDependencies, localModules, alreadyInstalledDependencies) {
  ...
  for (const requestedDependencyName in result) {
    for (const requestedDependencyVersionRange in result[requestedDependencyName]) {
      const resolvedVersionRange = moduletools.getResolvedVersion(requestedDependencyName, requestedDependencyVersionRange);
      ...
      for (const installedDependency of alreadyInstalledDependencies) {
        ...
        if (!semver.satisfies(installedDependency.version, resolvedVersionRange)) {
          continue;
        }
        ...
        for (const locationOfRequestingModule of result[requestedDependencyName][requestedDependencyVersionRange]) {
          ...
          logger.debug(`dependency ${requestedDependencyName}@${requestedDependencyVersionRange} (=${resolvedVersionRange}) requested by '${shortenedRequesterLocation}' will be satisfied by installed version ${installedDependency.version} in '${path.join(shortenedInstalledLocation)}'`);
        }
      ...
      for (const localModule of localModules) {
        ...
        console.log(localModule.version, resolvedVersionRange, semver.satisfies(localModule.version, resolvedVersionRange));
        if (!semver.satisfies(localModule.version, resolvedVersionRange)) {
          continue;
        }
        ...
        for (const locationOfRequestingModule of result[requestedDependencyName][requestedDependencyVersionRange]) {
          ...
          logger.debug(`dependency ${requestedDependencyName}@${requestedDependencyVersionRange} (=${resolvedVersionRange}) requested by '${shortenedRequesterLocation}' will be satisfied by local version ${localModule.version} in '${path.join(shortenedLocalLocation)}'`);
        }
        delete result[requestedDependencyName][requestedDependencyVersionRange];
        break;
      }
  ...
  for (const dependencyName in dependencies) {
    for (const dependencyVersionRange in dependencies[dependencyName]) {
      result.push({
        ...
        resolvedVersionRange: moduletools.getResolvedVersion(dependencyName, dependencyVersionRange),
        identifier: `${dependencyName}@"${dependencyVersionRange}"`,
        resolvedIdentifier: `${dependencyName}@"${moduletools.getResolvedVersion(dependencyName, dependencyVersionRange)}"`,
...
function determineDependencyTargetFolder(requestedDependencyArray, alreadyInstalledDependencies) {
  ...
  for (const requestedDependency of requestedDependencyArray) {
    ...
    const requestedVersionIsValidSemver = semver.validRange(requestedDependency.resolvedVersionRange);
    ...
    for (const possiblePathElement of possiblePathElements) {
      ...
      logger.debug(`found a place to install ${requestedDependency.identifier} (=${requestedDependency.resolvedVersionRange}): ${currentPath}`);
...
function removeContradictingInstalledDependencies() {
  ...
  return moduletools.getAllModulesAndInstalledDependenciesDeep()
    .then((result) => {
      ...
      for (const module of localModules) {
        for (const dependency in module.dependencies) {
          const requestedVersion = moduletools.getResolvedVersion(dependency, module.dependencies[dependency]);
...
function fixMissingDependenciesWithSymlinks() {
  ...
  return moduletools.getAllModulesAndInstalledDependenciesDeep()
    .then((result) => {
      ...
      for (const module of localModules) {
        for (const dependency in module.dependencies) {
          const resolvedDependencyVersion = moduletools.getResolvedVersion(dependency, module.dependencies[dependency]);
          ...
          for (const installedModule of installedDependencies) {
            ...
            if (installedModule.location === path.join(module.fullModulePath, 'node_modules')) {
              ...
              break;
            } else if (semver.satisfies(installedModule.version, resolvedDependencyVersion)) {
              fittingInstalledModule = installedModule;
            }
          }
          ...
          if ((!fittingInstalledModule || !dependencyAlreadyInstalled) && linkModules) {
            for (const localModule of localModules) {
              if (localModule.name !== dependency ||
                !semver.satisfies(localModule.version, resolvedDependencyVersion)) {
                continue;
              }
...
function parseProcessArguments() {
  for (let i = 2; i < process.argv.length; i++) {
    if (process.argv[i].indexOf('--') !== 0) {
      ...
      assumeLocalVersionsSatisfyNonSemverDependencyVersions = true;
