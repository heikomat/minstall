...
function findOptimalDependencyTargetFolder() {
  ...
  return moduletools.getAllModulesAndInstalledDependenciesDeep()
    .then((result) => {
      ...
      // remove all dependencies that wont get shadowed after the installation AND
      // are already satisfied by the current installation will be satisfied by linked local modules
    ...
    .then((result) => {
      ...
      for (const module of localModules) {
        for (const dependency in module.dependencies) {
          ...
          let fittingInstallationLocation = null;
          for (const installedModule of installedDependencies) {
            if (installedModule.name !== dependency ||
              !semver.satisfies(installedModule.version, module.dependencies[dependency])) {
              if (installedModule.name === dependency) {
                console.log(`semver mismatch for ${dependency} ON ${module.fullModulePath}: wanted ${module.dependencies[dependency]}, got ${installedModule.version} in ${installedModule.fullModulePath}`);
              }
              continue;
            }

            fittingInstallationLocation = installedModule.fullModulePath;
          ...
          if (!fittingInstallationLocation) {
            for (const localModule of localModules) {
              ...
              fittingInstallationLocation = localModule.fullModulePath;
              break;
            }
          }
          ...
          if (!dependencyAlreadyInstalled) {
            if (!fittingInstallationLocation) {
              logger.error(`NO INSTALLATION FOUND FOR DEPENDENCY ${dependency} ON ${module.fullModulePath}`);
            }
            // The Dependency is not localy installed
            symlinkPromises.push(systools.link(fittingInstallationLocation, path.join(module.fullModulePath, 'node_modules')));
          }
        }
      }
