...
function checkStartConditions() {
  ...
  return systools.verifyFolderName(process.cwd(), 'node_modules')
    ...
    .then((isSymlink) => {
      // Wenn node_modules kein symlink ist, muss minstall in den dependencies stehen
...
function installModules(installedModules, moduleInfos, index) {
  ...
  process.stdout.write(`- running postinstall for ${moduleDetails.folderName} (${index + 1}/${moduleInfos.length})`);
  ...
  const moduleNodeModules = path.join(process.cwd(), moduletools.modulesFolder, moduleDetails.folderName, 'node_modules');
  let packetsToKeep;
