...
function findOptimalDependencyTargetFolder() {
  ...
  return moduletools.getAllModulesAndInstalledDependenciesDeep()
    .then((result) => {
      ...
      // TODO: inform the user about non-optimal dependency-versions, like
      // when most modules want version x of a dependency, but a few want
      // version y of the same dependency
  ...
  return findOptimalDependencyTargetFolder()
    .then((targets) => {
      ...
      for (const targetFolder in targets) {
        ...
        console.log(`${installationIndex + 1}. installing ${targets[targetFolder].length} dependencies to ${shortTargetFolder}`);
