...
function installModuleDependencies() {
  ...
  // TODO: remove dead symlinks first. these were probably killed by a previous install with npm5
  // then test multple install-runs with npm5, see if 'enoent ENOENT: no such file or directory' happens
  // then test 1 clean install and then one where meta has gulp@^2.0.0 as dependency
  return removeContradictingInstalledDependencies()
    .then(() => {
      return removeDeadSymlinksInInstalledDependencies();
    })
    .then(() => {
