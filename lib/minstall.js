...
function findOptimalDependencyTargetFolder() {
  ...
  return moduletools.getAllModulesAndInstalledDependenciesDeep()
    .then((result) => {
      ...
      // the optimal folder is as close to the root folder as possible.
      // if some already installed dependency, or a dependency that will
      // be installed shadowes a higher install-folder, then the install-folder
      // cannot be further optimized

      // now we have our "to-install-list". next, check if any dependencies
      // would be shadowed by closer installs, and if so add even closer installs
      // to the list, so the correct version get installed everywhere.
      // repeat this until no more destructive shadowings are found
