...
function printNonOptimalDependencyInfos(requestedDependencies) {
  ...
  for (const requestedDependencyName in requestedDependencies) {
    ...
    const requestedByOtherPackagesString = requestedVersions.map((requestedVersion) => {
      ...
      return `|   version ${requestedVersion.versionRange} satisfies ${requestedVersion.requestedBy.length} local modules:
|     ${requestedByString}`;
    }).join('\n| ');

    logIfInRoot(`|
| ${mostRequested.requestedBy.length} local modules are satisfied with version ${mostRequested.versionRange} of ${mostRequested.name}, but some aren't:
...
|     ${requestedByString}`;
    ...
    logIfInRoot(`|
| you have version ${localVersionOfDependency.version} of ${localVersionOfDependency.name} localy, but some local modules request a different version:
${requestedByOtherPackagesString}`);
  }
