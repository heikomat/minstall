...
minstall will add a node_modules-folder to
   '${moduleFolder}'
and install the requested versions of the conflicting packages there.
Because of that, everything will work just fine, but you might want
to consider using the same package-versions throughout your whole project.
...
function installModules(installedModules, moduleInfos, index) {
  ...
  if (isInProjectRoot) {
    process.stdout.write(`- installing ${moduleInfo.folderName} (${moduleIndex + 1}/${moduleInfos.length})`);
  }
...
function run() {
  ...
  checkStartConditions()
    ...
    .then((modules) => {
      ...
      logIfInRoot(`running minstall. modules found: \n${moduletools.getModuleLogText(moduleInfos)}`);
