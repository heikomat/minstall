...
function determineDependencyTargetFolder(requestedDependencyArray, alreadyInstalledDependencies) {
  ...
  for (const requestedDependency of requestedDependencyArray) {
    ...
    const requestedVersionIsValidSemver = semver.valid(semver.coerce(requestedDependency.versionRange));
    if (!requestedVersionIsValidSemver) {
      ...
      logger.warn(`${requestedDependency.requestedBy.length} modules request ${requestedDependency.identifier}. These dependency-installs won't get optimized (they won't get hoisted or linked), Because '${requestedDependency.versionRange}' is not a vaild semver-version-rage:\n ${requestedDependency.requestedBy.join('\n ')}`);
