...
const moduletools = {
    ...
    for (let name in moduleInfo.dependencies) {
      ...
      if (installedIndex >= 0) {
        ...
        if (!semver.satisfies(checkVersion, targetRange)) {
          ...
          messages.push(`installed package ${name}@${checkVersion} doesn't fit the dependency ${name}@${targetRange} from ${moduleInfo.name}`)
        }
      } else {
        missingDependencies.push({name: name, version: moduleInfo.dependencies[name]});
      }
  ...
  installPackets(packets) {
    ...
    return systools.runCommand(`npm install --loglevel=error ${identities.join(' ')}`)
      .catch((error) => {
